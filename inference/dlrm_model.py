...
class DLRMDCNV2(nn.Module):
    ...
    @nn.compact
    def __call__(self, dense_features, dense_lookups, embedding_lookups):
        dense_outputs = self.bottom_mlp(dense_features)
        dense_embeddings = []
        processed_dense_lookups = []
        for key, value in dense_lookups.items():
            embeddings = nn.Embed(
                self.vocab_sizes[int(key)], self.embedding_size, name=f"Embed_{key}"
            )(value)
        ...
        if processed_dense_lookups:
            ...
            stacked_dense_embeddings = jnp.empty(
                (self.global_batch_size, 0, self.embedding_size)
            )
        ...
        interaction_args = jax.lax.concatenate(
            [
                dense_outputs.reshape((self.global_batch_size, 1, self.embedding_size)),

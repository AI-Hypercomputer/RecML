...
class KerasOrbaxCheckpointUtilsTest(absltest.TestCase):
  ...
  def test_restore_partial_checkpoint_with_transforms(self):
    manager = keras_utils.KerasOrbaxCheckpointManagerV3(
        checkpoint_dir=self.checkpoint_dir,
        max_to_keep=1,
        save_interval_epochs=1,
    ...
    # Source model has a dense layer named "source_dense"
    model_src = keras.Sequential(
        [keras.layers.Dense(2, name="source_dense")],
        name="source_model",
    )
    model_src.compile(optimizer="adam")
    model_src.build((1, 1))
    model_src.optimizer.build(model_src.trainable_variables)
    test_kernel = np.array([[3.0, 5.0]], dtype=np.float32)
    test_bias = np.array([1.5, 2.5], dtype=np.float32)
    model_src.layers[0].kernel.assign(test_kernel)
    model_src.layers[0].bias.assign(test_bias)

    manager.save_model_variables(model_src, epoch=1)
    manager.wait_until_finished()

    # Target model has a dense layer with a different name / path "target_dense"
    model_dst = keras.Sequential(
        [keras.layers.Dense(2, name="target_dense")],
        name="target_model",
    )
    model_dst.build((1, 1))
    model_dst.layers[0].kernel.assign(np.zeros((1, 2), dtype=np.float32))
    model_dst.layers[0].bias.assign(np.zeros((2,), dtype=np.float32))

    partial_vars = {
        keras_utils.TRAINABLE_VARIABLES_KEY: {
            "target_model/target_dense/kernel": model_dst.layers[0].kernel,
            "target_model/target_dense/bias": model_dst.layers[0].bias,
        }
    }

    # Provide explicit transforms to map target variable paths to source
    # checkpoint paths.
    transforms = {
        keras_utils.TRAINABLE_VARIABLES_KEY: {
            "target_model/target_dense/kernel": ocp.transform_utils.Transform(
                original_key=(
                    f"{keras_utils.TRAINABLE_VARIABLES_KEY}/"
                    "source_model/source_dense/kernel"
                )
            ),
            "target_model/target_dense/bias": ocp.transform_utils.Transform(
                original_key=(
                    f"{keras_utils.TRAINABLE_VARIABLES_KEY}/"
                    "source_model/source_dense/bias"
                )
            ),
        }
    }
    ...
    np.testing.assert_allclose(model_dst.layers[0].kernel.value, test_kernel)
    np.testing.assert_allclose(model_dst.layers[0].bias.value, test_bias)
    manager.close()
  ...
  def test_restore_v3_missing_metadata_file_fails(self):
    manager = keras_utils.KerasOrbaxCheckpointManagerV3(
        checkpoint_dir=self.checkpoint_dir,
        max_to_keep=1,
        save_interval_epochs=1,
    ...
    # Remove non_trainable_paths metadata directory
    nt_path = os.path.join(
        self.checkpoint_dir, "1", keras_utils.NON_TRAINABLE_PATHS_KEY
    )
    self.assertTrue(os.path.exists(nt_path))
    shutil.rmtree(nt_path)

    with self.assertRaisesRegex(
        ValueError,
        "missing required checkpoint item",
    ):
      keras_utils.restore_keras_checkpoint(
          self.checkpoint_dir,
          model=model,
  ...
  def test_validate_v3_checkpoint_missing_state_key_fails(self):
    step_dir = os.path.join(self.checkpoint_dir, "1")
    os.makedirs(step_dir, exist_ok=True)
    os.makedirs(
        os.path.join(step_dir, keras_utils.STATE_CHECKPOINT_KEY), exist_ok=True
    )
    for meta_key in (
        keras_utils.NON_TRAINABLE_PATHS_KEY,
        keras_utils.OPTIMIZER_PATHS_KEY,
    ):
      with open(os.path.join(step_dir, meta_key), "w") as f:
        f.write('{"paths": []}')

    incomplete_metadata = {
        keras_utils.TRAINABLE_VARIABLES_KEY: {},
        # Missing NON_TRAINABLE_VARIABLES_KEY and OPTIMIZER_VARIABLES_KEY
    }
    with self.assertRaisesRegex(
        ValueError,
        "missing required state keys in checkpoint metadata",
    ):
      keras_utils._validate_v3_checkpoint(step_dir, incomplete_metadata)

  def test_extract_variable_aliases(self):
    ...
    aliases = keras_utils.extract_variable_aliases(model)
    expected_aliases = {
  ...
  def test_extract_variable_aliases_no_shared_variables(self):
    model = keras.Sequential(
        [
    ...
    model.build((1, 8))
    aliases = keras_utils.extract_variable_aliases(model)
    self.assertEmpty(aliases)

  def test_extract_variable_aliases_multi_surface(self):
    shared_dense = keras.layers.Dense(2, name="shared_dense")
  ...
  def test_extract_variable_aliases_deeply_nested(self):
    shared_dense = keras.layers.Dense(2, name="shared_dense")
  ...
  def test_restore_partial_checkpoint_with_alias_graph(self):
    manager = keras_utils.KerasOrbaxCheckpointManagerV3(
        checkpoint_dir=self.checkpoint_dir,
        max_to_keep=1,
        save_interval_epochs=1,
    ...
    # Restores successfully using the alias graph automatically
    restored_state = keras_utils.restore_partial_checkpoint(
        self.checkpoint_dir, partial_vars, epoch=1
    )
  ...
  def test_restore_partial_checkpoint_explicit_transforms_precedence(self):
    ...
    # Explicit transform mapping target to custom_emb instead of alias graph
    # default (chrome_surface).
    explicit_transforms = {
        keras_utils.TRAINABLE_VARIABLES_KEY: {
            model_target.target_emb.embeddings.path: (
  ...
  def test_restore_keras_checkpoint_v3_with_alias_resolution(self):
    manager = keras_utils.KerasOrbaxCheckpointManagerV3(
        checkpoint_dir=self.checkpoint_dir,
        max_to_keep=1,
        save_interval_epochs=1,
  ...
  def test_restore_v3_checkpoint_missing_variable_aliases_metadata_succeeds(
      self,
  ):
    manager = keras_utils.KerasOrbaxCheckpointManagerV3(
        checkpoint_dir=self.checkpoint_dir,
    ...
    # Remove variable_aliases metadata to simulate pre-alias V3 checkpoint.
    alias_path = os.path.join(
        self.checkpoint_dir, "1", keras_utils.VARIABLE_ALIASES_KEY
    )

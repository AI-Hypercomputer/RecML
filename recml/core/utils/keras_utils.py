...
def _variables_to_path_dict(
  ...
  if duplicates:
    raise ValueError(
        ...
        f"unique layer names (e.g. set name_layers=True if using a GUM "
...
def _detect_checkpoint_version(checkpoint_path: str) -> int:
...
def is_v3_checkpoint(checkpoint_dir: str, epoch: int | None = None) -> bool:
  ...
  return _detect_checkpoint_version(checkpoint_path) == 3


def is_v1_checkpoint(checkpoint_dir: str, epoch: int | None = None) -> bool:
  """Checks if a checkpoint is in V1 format."""
  checkpoint_path, _ = resolve_orbax_checkpoint_path(checkpoint_dir, epoch)
  return _detect_checkpoint_version(checkpoint_path) == 1
...
def _prepare_v3_restore(
  ...
  has_saved_paths = (
      epath.Path(checkpoint_path) / NON_TRAINABLE_PATHS_KEY
  ).exists()
  non_trainable_paths = None
  optimizer_paths = None
  if has_saved_paths and model is not None:
  ...
  for key in keys:
    ...
    elif key in abstract_state and abstract_state[key]:
      raise ValueError(
  ...
  for key in [
    ...
    if key in variables:
      ...
      if restored_var_dict is not None:
        ...
        assert not var_dict, f"Key {key} not found in restored state."
  if restore_optimizer_vars:
    key = OPTIMIZER_VARIABLES_KEY
    var_dict = variables[key]
    restored_var_dict = restored_state.get(key)
    assert restored_var_dict is not None, (

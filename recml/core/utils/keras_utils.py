...
def is_v1_checkpoint(checkpoint_dir: str, epoch: int | None = None) -> bool:
  ...
  checkpoint_path, _ = resolve_orbax_checkpoint_path(checkpoint_dir, epoch)
  return _detect_checkpoint_version(checkpoint_path) == CheckpointVersion.V1
...
def _prepare_v3_restore(
  ...
  # Validate that required keys present in model exist in checkpoint metadata.
  for key in keys:
    if abstract_state.get(key) and key not in saved_state_metadata:
      raise ValueError(
          f"Key {key} not found in checkpoint metadata, but model has "
          "variables to restore."
      )

  # Load index path metadata for non-trainable and optimizer variables.
  non_trainable_paths = None
  if (
      model is not None
      and abstract_state.get(NON_TRAINABLE_VARIABLES_KEY)
      and NON_TRAINABLE_VARIABLES_KEY in saved_state_metadata
  ):
    nt_path = epath.Path(checkpoint_path) / NON_TRAINABLE_PATHS_KEY
    if not nt_path.exists():
      raise ValueError(
          f"Index metadata file '{NON_TRAINABLE_PATHS_KEY}' not found in"
          f" checkpoint {checkpoint_path}. Unable to perform index-based"
          " mapping for non-trainable variables."
      )
  ...
  if (
      ...
      and OPTIMIZER_VARIABLES_KEY in saved_state_metadata
  ):
    opt_path = epath.Path(checkpoint_path) / OPTIMIZER_PATHS_KEY
    if not opt_path.exists():
      raise ValueError(
          f"Index metadata file '{OPTIMIZER_PATHS_KEY}' not found in checkpoint"
          f" {checkpoint_path}. Unable to perform index-based mapping for"
          " optimizer variables."
      )
    checkpointer = ocp.Checkpointer(ocp.handlers.JsonCheckpointHandler())
  ...
  for key in keys:
    if not abstract_state.get(key) or key not in saved_state_metadata:
      continue
      ...
      if restored_var_dict is not None:
        for path, var in var_dict.items():
          if path in restored_var_dict:
            ...
            raise ValueError(
                f"Path {path} not found in restored state for key {key}."
            )
      else:
        if var_dict:

...
This work has been published at [PLACE_HOLDER].
"""
...
def _auto_block_v(n: int, vocab_size: int) -> int:
  """Automatically picks block_v to target ~32MB intermediate logits."""
  ...
  # Target 32MB for intermediate logits tensor [local_n, block_v]
  # in float32 (4 bytes).
  target_elements = (32 * 1024 * 1024) // 4
  block_v = target_elements // local_n

  # Align to MXU size (128)
  block_v = max(128, (block_v // 128) * 128)

  # Clamp to reasonable range [256, 8192]
  block_v = max(256, min(8192, block_v))
  # Don't exceed vocab_size
  block_v = min(vocab_size, block_v)
  return block_v


def _get_sharding(x) -> jax.sharding.Sharding | None:
...
def _bce_fwd_local(
  ...
  if config.compute_metrics:
    ...
    # TODO(yaoyiran): Abstract the shared logits and base loss computation.
    def v_body(
...
def cut_binary_cross_entropy(
  ...
  Args:
    ...
    pure_jax: If True, use pure JAX implementation; if False, use Pallas
      kernels.
...
def _max_safe_block_v(vmem_budget: int, padded_d: int) -> int:
  ...
  # On 128MB VMEM (TPU v5e/v6e), max block_v * padded_d is 1048576.
  if vmem_budget <= 32 * 1024 * 1024:
    max_product = 131072
    max_absolute = 1024
  else:
    max_product = 16777216
    max_absolute = 32768

  # NOMUTANTS -- max_safe is calculated based on VMEM budget.
  max_safe = max_product // padded_d
  # Ensure it is a multiple of 128
  max_safe = (max_safe // 128) * 128
  return max(256, min(max_safe, max_absolute))
...
def _max_safe_chunk_n(
    vmem_budget: int, padded_d: int, target_ratio: float | None = None
) -> int:
  """Returns maximum safe chunk_n to prevent VMEM OOM."""
  if target_ratio is None:
    target_ratio = _TARGET_RATIO
  # Conservative VMEM usage approx: chunk_n * padded_d * 4 * 3.
  ...
  max_safe = target_bytes // (padded_d * 4 * 3)
  # Round down to multiple of 128 for MXU alignment
  max_safe = (max_safe // 128) * 128

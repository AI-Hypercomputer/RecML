...
#   JAX scan loop. On <=32 MB chips, this falls back to 0.3.
#
# _BLOCK_N:
...
#   directly to full vector registers without partial-lane edge masking.
...
def set_hyperparams(
  ...
  """Sets the global hyper-parameters for Cut BCE VMEM allocation."""
  global _RESERVE_MB, _TARGET_RATIO, _BLOCK_N
  if reserve_mb is not None:
    _RESERVE_MB = float(reserve_mb)
  if target_ratio is not None:
    _TARGET_RATIO = float(target_ratio)
  if block_n is not None:
    _BLOCK_N = int(block_n)
...
def cut_binary_cross_entropy(
  ...
  if return_metrics:
    losses, tp, fp, fn, tn = _cut_binary_cross_entropy(
        BCEConfig(
            block_v=block_v,
    ...
    losses = _cut_binary_cross_entropy(
        BCEConfig(
            block_v=block_v,
...
def _pallas_vmem_budget(reserve_mb: float | None = None) -> int:
  ...
  if any(d.platform == "tpu" for d in jax.devices()):
    ...
    return max(16 * 1024 * 1024, cap)
  return 16 * 1024 * 1024


def _max_safe_block_v(vmem_budget: int, padded_d: int) -> int:
  """Returns maximum safe block_v to prevent VMEM OOM."""
  # On 16MB VMEM (TPU v3/v4), max block_v * padded_d is 131072.
  # On 64MB and 128MB VMEM TPUs, max block_v * padded_d is 1048576.
  if vmem_budget <= 32 * 1024 * 1024:
    max_product = 131072
    max_absolute = 1024
  else:
    max_product = 1048576
  ...
  max_safe = max_product // padded_d
  # Ensure it is a multiple of mxu_size
  mxu_size = _get_mxu_size()
  max_safe = (max_safe // mxu_size) * mxu_size
  return max(mxu_size, min(max_safe, max_absolute))
...
def _check_vocab_replicated_in_d(emb_spec: jax.sharding.PartitionSpec) -> None:
  if len(emb_spec) > 1 and emb_spec[1] is not None:
    raise NotImplementedError(
...
def _bce_bwd_kernel(
    ...
    act_ref: jax.Ref,  # [padded_n, padded_d] HBM
    tgt_ref: jax.Ref,  # [l_padded, padded_n] HBM
    dloss_ref: jax.Ref,  # [l_padded, padded_n] HBM
    d_emb_ref: jax.Ref,  # [block_v, padded_d] HBM output
    d_act_partials_ref: jax.Ref,  # [1, padded_n, padded_d] HBM output
  ...
  # Precomputed inverse vocab and batch iota
  inv_vocab = 1.0 / vocab
...
def _max_safe_chunk_n(
  ...
  bytes_per_element = jnp.dtype(dtype).itemsize
  if vmem_budget <= 32 * 1024 * 1024:
    target_bytes = int(vmem_budget * 0.3)
  else:
    target_bytes = int(vmem_budget * target_ratio)
  max_safe = target_bytes // (padded_d * bytes_per_element * 3)

  # Round down to multiple of 128 to match hardware vector register lane size
  # (which is 128 across all TPU generations, distinct from the MXU systolic
  # array column dimension which can be 128 or 256).
  max_safe = (max_safe // 128) * 128

  # Clamped to [512, 32768]:
  # - Floor 512: Ensures chunk is large enough (4 x 128 lanes) to amortize
  #   kernel launch and DMA dispatch overhead.
  # - Ceiling 32768: Caps max chunk size on large VMEM chips to prevent a single
  #   chunk from monopolizing VMEM and squeezing out the vocabulary dimension
  #   tiles (block_v), intermediate logits, and weights.
  return max(512, min(max_safe, 32768))
...
def _bce_bwd_pure_jax(
  ...
  if activations.ndim == 4:
    ...
    for g in range(groups):
      ...
      d_act_g, d_emb_g = _bce_bwd_pure_jax(
...
def _bce_bwd_loop_fallback(
  ...
  for g in range(groups):
    tgt_g = targets if targets.ndim == 3 else targets[g]
    d_act_g, d_emb_g = _bce_bwd_pallas_chunked_n(
        config,
        d_loss[g],
        activations[g],
        embeddings,

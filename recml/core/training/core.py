...
_JOB_INFO_ATTR = "____job_info____"


@dataclasses.dataclass(frozen=True)
class JobInfo:
  """Job information for the trainer."""

  name: str
  mode: Any

  @classmethod
  def from_string(cls, job_info: str) -> Self:
    """Creates a JobInfo from a string."""
    return cls(**json.loads(job_info))

  def to_string(self) -> str:
    """Converts the JobInfo to a string."""
...
class Trainer(abc.ABC, Generic[TaskT]):
  ...
  @property
  def job_info(self) -> JobInfo | None:
    """Returns the job info string."""
    return getattr(self, _JOB_INFO_ATTR, None)

  def set_job_info(self, job_info: str):
    """Sets the job info string."""
    object.__setattr__(self, _JOB_INFO_ATTR, JobInfo.from_string(job_info))

...
class KerasTask(abc.ABC):
  ...
  @abc.abstractmethod
  def create_dataset(self, training: bool, **kwargs) -> tf.data.Dataset:
    """Creates a training or evaluation dataset.

    Args:
      training: A flag indicating whether the dataset is for training or
        evaluation.
      **kwargs: Optional keyword arguments for dataset creation. Can be one of
        the following, `eval_name`- the name of the evaluation job, which will
        be used as a suffix for the evaluation logging directory.
...
class KerasTrainer(core.Trainer[KerasTask]):
  ...
  def __init__(
      ...
      *,
      ...
      checkpoint_version: keras_utils.CheckpointVersion | str | None = None,
    ...
    Args:
      ...
      checkpoint_version: The checkpoint version to use. Supported versions:
        "v1" (legacy V1), "v2" (V2), "v3" (V3). If None, falls back to
        legacy_checkpoint_format (defaulting to V1).
    ...
    if checkpoint_version is not None:
    ...
    elif legacy_checkpoint_format is not None:
      logging.warning(
      ...
      self._checkpoint_version = keras_utils.CheckpointVersion.V1
    ...
    if keras.backend.backend() == "jax":
      return [
          keras_utils.EpochSummaryCallback(
              ...
              # Allow eval jobs to have separate logging directories.
              eval_subdir=(
                  self.job_info.name
  ...
  def _maybe_get_dataset_kws(self, task: KerasTask) -> Mapping[str, Any]:
    kws = {}
    if py_utils.has_argument(task.create_dataset, "eval_name"):
      kws["eval_name"] = (
          self.job_info.name if self.job_info is not None else "validation"
      )
    if py_utils.has_argument(task.create_dataset, "model_dir"):
      kws["model_dir"] = self._model_dir

    return kws

  def _maybe_get_model_kws(
      self, task: KerasTask, dataset: tf.data.Dataset, *, val: bool
  ) -> Mapping[str, Any]:
    kws = {}
    ...
    if (
        val
        and py_utils.has_argument(task.create_model_for_eval, "eval_name")
        and (
            py_utils.has_argument(task.create_model, "eval_name")
            or task.create_model_for_eval.__func__
            is not KerasTask.create_model_for_eval
        )
    ):
      kws["eval_name"] = (
          self.job_info.name if self.job_info is not None else "validation"
  ...
  def train(self, task: KerasTask) -> core.Logs:
    ...
    dataset = task.create_dataset(
        training=True, **self._maybe_get_dataset_kws(task)
    )
  ...
  def evaluate(self, task: KerasTask) -> core.Logs:
    ...
    dataset = task.create_dataset(
        training=False, **self._maybe_get_dataset_kws(task)
    )

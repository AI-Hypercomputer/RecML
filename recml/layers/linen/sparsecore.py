...
class SparsecorePreprocessor:
  ...
  def __call__(
    ...
    weights = {}
    has_weights = any(
        self.sparsecore_config.specs[key].weight_name is not None
    ...
    for key in self.sparsecore_config.feature_specs:
      ...
      spec = self.sparsecore_config.specs[key]
      if spec.weight_name is not None:
        weights[key] = _to_np(inputs[spec.weight_name])
        sparse_features.add(spec.weight_name)
      elif has_weights:
    ...
    preprocessed_inputs, _ = embedding.preprocess_sparse_dense_matmul_input(
        features=features,
        features_weights=weights if has_weights else None,

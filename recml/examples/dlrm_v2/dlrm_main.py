...
from dataloader import CriteoDataLoader, DataConfig
from dlrm_model import DLRMDCNV2, uniform_init
from jax_tpu_embedding.sparsecore.lib.flax import embed, embed_optimizer
...
_EVAL_STEPS = flags.DEFINE_integer(
    "eval_steps", 100, "Number of steps to run for each evaluation."
)
...
def eval_loop(
  ...
  for _ in range(eval_steps):
    try:
      labels, dense_features, dense_lookups, embedding_lookups = next(
          eval_producer
      )
      eval_metrics_collection = eval_step_fn(
          apply_fn,  # Pass apply_fn to the jitted function
          params,
          labels,
          dense_features,
          dense_lookups,
  ...
  try:
    ...
    auc_val = 0.5  # Default if AUC cannot be computed
  logging.info(
...
def train_loop(model: DLRMDCNV2, feature_specs: Mapping[str, embedding_spec.FeatureSpec], mesh: jax.sharding.Mesh):
  ...
  # --- Checkpointing Setup and Restore ---
  checkpoint_dir = os.path.join(_MODEL_DIR.value, "checkpoints")
  checkpointer = ocp.CheckpointManager(checkpoint_dir, ocp.PyTreeCheckpointer())

  initial_step = 0
  if _RESTORE_CHECKPOINT.value:
    latest_step = checkpointer.latest_step()
    if latest_step is not None:
        logging.info("Found checkpoint at step %d. Restoring...", latest_step)
        ...
        logging.info("Restored state from step %d.", initial_step)
    else:
        logging.info("No checkpoint found to restore from. Starting from scratch.")
  ...
  for step in range(initial_step, _NUM_STEPS.value):
    ...
    with jax.profiler.StepTraceAnnotation("train_step", step_num=step):
      ...
      params, opt_state, train_metrics_collection = train_step(
          params, labels, dense_features, dense_lookups, embedding_lookups,
          opt_state, train_metrics_collection
      )

    # Logging
    if (step + 1) % _LOGGING_INTERVAL.value == 0:
      end_time = time.time()
      ...
      logging.info(
          ...
          step + 1, loss_val, accuracy_val, auc_val,
    ...
    # Evaluation
    if (step + 1) % _EVAL_INTERVAL.value == 0 and _EVAL_FILE_PATTERN.value:
      eval_producer = DLRMDataLoader(
      ...
      eval_loop(_EVAL_STEPS.value, eval_producer, eval_step, params, model.apply)
      eval_producer.stop()

    # Checkpointing
    if (step + 1) % _SAVE_CHECKPOINT_INTERVAL.value == 0:
        ckpt_to_save = {"params": params, "opt_state": opt_state, "step": step + 1}
        save_args = ocp.args.PyTreeSave(ckpt_to_save)
        checkpointer.save(step + 1, args=save_args, force=True)
        logging.info("Saved checkpoint for step %d", step + 1)
...
def main(argv):
  ...
  # Start the main loop.
  train_loop(model, feature_specs, mesh)

...
class StuJaxTest(parameterized.TestCase):
  ...
  def test_output_shape_and_gradients(self, config_dict):
    ...
    stu_configs = [config for _ in range(config_dict['num_layers'])]
    model = STUStack(configs=stu_configs)
    ...
    def loss_fn(p, x, num_targets, rng_key):
      y = model.apply(
          {'params': p, 'cache': variables['cache']},
          x,
    ...
    apply_fn = jax.jit(
        lambda v, x, num_targets: model.apply(
            v, x, num_targets=num_targets
        ),
        out_shardings=self.batch_sharding,
    )

    output = apply_fn(variables, dummy_x, dummy_num_targets)
    self.assertEqual(output.shape, dummy_x.shape)
    self.assertEqual(output.sharding, self.batch_sharding)

    grads = jax.grad(loss_fn)(
        variables["params"], dummy_x, dummy_num_targets, dropout_key
    )
  ...
  def test_target_invariance(self):
    ...
    output_original = apply_fn(variables, x, num_targets)
    ...
    output_swapped_input = apply_fn(variables, swapped_x, num_targets)
    ...
    np.testing.assert_allclose(
        output_original, output_swapped_restored, rtol=1e-2, atol=1e-2
    )

  def test_kv_caching(self):
    """Tests that decode with KV caching is equivalent to running without cache."""
    if jax.devices()[0].platform != 'tpu':
      self.skipTest('Test only supported on TPUs.')

    batch_size, max_len, embedding_dim = 2, 8, 16
    config = STULayerConfig(
        embedding_dim=embedding_dim,
        num_heads=2,
        hidden_dim=24,
    ...
    init_key, data_key, dropout_key = jax.random.split(self.key, 3)
    x = jax.random.normal(data_key, (batch_size, max_len, embedding_dim))
    num_targets = jnp.ones((batch_size,), dtype=jnp.int32)
    ...
    y_decoded_list = []
    current_vars = decode_variables
    for i in range(max_len):
      y_i, mutated_vars = model.apply(
          current_vars,
          x[:, i : i + 1, :],
          num_targets=num_targets,
          deterministic=True,
      ...
      current_vars = core.freeze(
          {'params': current_vars['params'], **mutated_vars}
      )
      y_decoded_list.append(y_i)

    y_decoded = jnp.concatenate(y_decoded_list, axis=1)

    np.testing.assert_allclose(y_decoded, y_ref, rtol=0.25, atol=0.25)

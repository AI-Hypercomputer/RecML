...
def _get_supervision_labels_and_weights(
    ...
    candidate_padding_mask: jnp.ndarray,
  ...
  for task in task_configs:
    ...
    supervision_weights[task.task_name] = candidate_padding_mask.astype(
        jnp.float32
    )
...
class DlrmHSTU(nn.Module):
  ...
  def setup(self):
    self._embedding_layer = sparsecore.SparsecoreEmbed(
        self.hstu_configs.sparsecore_config,
        mesh=self.mesh,
    )
    ...
    for _ in range(hstu_config.hstu_attn_num_layers):
      ...
      stu_configs.append(stu_layer_config)
    stu_module = STUStack(configs=stu_configs)
  ...
  def _construct_payload(
    ...
    for (
      ...
      is_sc_feature = (
          uih_name in self.hstu_configs.sparsecore_config.specs
      )
      # Handle embedding features that need to be in the payload.
      if uih_name in embeddings:
        if uih_name not in payload:
          if is_sc_feature:
            payload[uih_name] = embeddings[uih_name]
          else:
            payload[uih_name] = self._concat_features(
                embeddings[uih_name], embeddings[cand_name]
            )
      # Handle non-embedding features that need to be merged.
      elif uih_name in uih_features and cand_name in cand_features:
        if uih_name not in payload:
          payload[uih_name] = uih_features[uih_name]
    ...
    if self.hstu_configs.target_enrich_features:
      for feat_name in self.hstu_configs.target_enrich_features:
        if feat_name in embeddings and feat_name not in payload:
          payload[feat_name] = embeddings[feat_name]
    return payload
  ...
  def __call__(
    ...
    Args:
        features: Dict of dense feature tensors. When using SparseCore, this
          dictionary is the output of SparsecorePreprocessor and must contain
          concatenated sequences for sparse features, along with other dense
          features like 'action_time', 'query_time' etc.
    ...
    """
    max_uih_len = features[
        self.hstu_configs.uih_action_time_feature_name
    ].shape[1]
    max_candidates = features[
        self.hstu_configs.candidates_querytime_feature_name
    ].shape[1]
    embeddings: Any = self._embedding_layer(features)

    candidate_padding_mask = jnp.arange(max_candidates) < num_candidates[:, None]

    if self.hstu_configs.uih_post_id_feature_name not in embeddings:
      raise ValueError(
          "Post ID feature "
    ...
    hstu_seq_embeddings = embeddings[
    ...
    hstu_seq_timestamps = self._concat_features(
        features[self.hstu_configs.uih_action_time_feature_name],
        features[candidate_querytime_feature_name],
    )
    ...
    supervision_bitmasks = features[
        self.hstu_configs.candidates_weight_feature_name
    ]
    watchtime_sequence = features[

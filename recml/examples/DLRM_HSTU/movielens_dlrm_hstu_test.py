...
class DlrmMovielensTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    sc_config = sparsecore.SparsecoreConfig(
        specs={
            'user_id': sparsecore.EmbeddingSpec(
                input_dim=self.user_vocab,
                embedding_dim=self.embed_dim,
                max_sequence_length=seq_len,
            ),
            'movie_id': sparsecore.EmbeddingSpec(
                input_dim=self.movie_vocab,
                embedding_dim=self.embed_dim,
                max_sequence_length=seq_len,
            ),
            'rating': sparsecore.EmbeddingSpec(
                input_dim=self.rating_vocab,
                embedding_dim=self.embed_dim,
                max_sequence_length=seq_len,
            ),
        },
  ...
  def test_forward_pass(self, deterministic):
    ...
    features['movie_id'] = np.concatenate(
        [uih_features['movie_id'], candidate_features['movie_id']], axis=1
    )
    features['user_id'] = np.concatenate(
        [uih_features['user_id'], candidate_features['user_id']], axis=1
    )
    features['rating'] = np.concatenate(
        [
            uih_features['rating'],
            np.zeros(
                (self.batch_size, self.max_candidates),
                dtype=uih_features['rating'].dtype,
            ),
        ],
        axis=1,
    )
    features['uih_weight'] = np.concatenate(
        [uih_features['uih_weight'], candidate_features['candidates_weight']],
        axis=1,
    )
